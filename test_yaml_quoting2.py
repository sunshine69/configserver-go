...
def quote_for_yaml(value):
    ...
    # Choose quote style - prefer double quotes, fall back to single
    has_double_quote = '"' in value
    has_single_quote = "'" in value
    
    if has_double_quote and not has_single_quote:
        # Use single quotes (escape inner single quotes by doubling)
        escaped = value.replace("'", "''")
        return f"'{escaped}'"
    
    # Default: use double quotes (escape backslashes and double quotes)
...
def process_ciphers_safe(content):
    ...
    def replace_cipher(match):
        ...
        decrypted = "P@ssw0rd!#\$%^&*()=+\"test's value"
